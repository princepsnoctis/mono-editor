...
const Directory = (props: DirectoryType) => {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (!menuRef.current?.contains(e.target as Node)) setMenuVisible(false);
    };
  ...
  const handleEditKeyPress: React.KeyboardEventHandler<HTMLInputElement> = (event) => {
    if (event.key == "Enter") {
      ...
      const updateFileName = (file: any, path: string, newName: string) => {
        ...
        if ("path" in file && file.path === path) {
          return {
            ...file,
            name: newName,
            path: `${path.split("/").slice(0, -1).join("/")}/${editedText}`,
          };
        }
        ...
        if ("children" in file) {
          return {
            ...file,
            children: file.children.map((child: DirectoryType) =>
              updateFileName(child, path, newName)
      ...
      setOpenedFiles((prev) => prev.map((file) =>
          "path" in file && file.path == props.path
            ? {
                ...file,
                name: editedText,
                path: `${props.path
  ...
  const children = opened ? props.children?.map((child, index) => {
    if (child.type == "directory") return <Directory key={index} {...child} depth={depth + 1} />;
    else return <File key={index} {...child} depth={depth + 1} />;
  }) : null;
  ...
  return (
    <>
      <div
        className="directory"
        onClick={() => {
          if (!isEditing && !menuVisible) setOpened((prev) => !prev);
        }}
      ...
      >
        <div className="arrow">
          <img
            ...
            style={{
              width: OpenedDirectory ? "10px" : "6px",
              height: OpenedDirectory ? "10px" : "6px",
            }}
            src={opened ? OpenedDirectory : ClosedDirectory}
            alt="Opened or closed directory"
          ></img>
        </div>
        <img
          className="file-icon"
      ...
      {isCreating[0] && (
        <input
          ...
          type="text"
          placeholder={isCreating[1] == "file" ? "File name" : "Directory name"}
